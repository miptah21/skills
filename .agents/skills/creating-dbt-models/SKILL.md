...
- [ ] Project conventions discovered (read 2-3 existing models)
- [ ] `dbt build` passes (not just compile)
- [ ] Output verified via `dbt show` (column names, row count, values)
- [ ] Calculations cross-checked against sample data
- [ ] Column names match requirements exactly
- [ ] Grain matches what was requested (one row per what?)
- [ ] No unexpected NULLs in required columns
- [ ] schema.yml entry added with at least primary key tests
