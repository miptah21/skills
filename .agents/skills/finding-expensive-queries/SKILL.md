...
Use these to classify query severity when user doesn't specify:

| Metric | Normal | Slow | Critical |
|--------|--------|------|----------|
| Execution time | < 60s | 60–300s | > 300s (5 min) |
| Data scanned | < 1 GB | 1–10 GB | > 10 GB |
| Partition pruning | < 25% scanned | 25–75% | > 75% (near full scan) |
| Spillage | 0 GB | < 5 GB local | > 5 GB or any remote |
| Credit cost (per query) | < 0.1 | 0.1–1.0 | > 1.0 |

See [references/THRESHOLDS.md](references/THRESHOLDS.md) for detailed classification tables and default time ranges.

## Validation Checklist
- [ ] Time period clarified with user
- [ ] QUERY_ATTRIBUTION_HISTORY queried for cost ranking
- [ ] Top queries enriched with QUERY_HISTORY stats
- [ ] Patterns identified (no pruning, repeated queries, spillage)
- [ ] Results ranked with severity classification
- [ ] Top 3-5 actionable recommendations provided
