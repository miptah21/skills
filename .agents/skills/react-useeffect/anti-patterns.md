...
```tsx
// ❌ Extra state + Effect for derived value
function Form() {
...
// ✅ Filter during render (memoize if expensive)
function TodoList({ todos, filter }) {
...
```tsx
// ❌ Effect for button click result
...
```tsx
// ❌ Child fetches, passes up via Effect
function Parent() {
...
```tsx
// ❌ Runs twice in dev, may break auth
function App() {
  useEffect(() => {
