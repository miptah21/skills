...
- [ ] Full error message read and understood
- [ ] Upstream models checked for breaking changes
- [ ] Fix applied to correct file (not just symptoms)
- [ ] `dbt build` passes (not just compile)
- [ ] Output verified via `dbt show` (data looks correct)
- [ ] `dbt test` passes for the model
- [ ] Downstream models rebuilt successfully (`dbt build --select model+`)
- [ ] Original requirement re-read and confirmed met
